...
app.put("/api/accounts/:id", async (c) => {
  ...
  const [result] = await db.update(schema.accounts).set(update)
...
app.get("/api/transactions/:id", async (c) => {
  ...
  const [row] = await db.select().from(schema.transactions)
...
app.put("/api/transactions/:id", async (c) => {
  ...
  const [result] = await db.update(schema.transactions).set(update)
...
app.put("/api/invoices/:id", async (c) => {
  ...
  const [result] = await db.update(schema.invoices).set(update)
...
app.get("/api/tax-codes", async (c) => {
  ...
  const rows = await db.select().from(schema.taxCodes)
    .where(and(...conditions))
...
app.get("/api/tax-codes/:id", async (c) => {
  ...
  const [row] = await db.select().from(schema.taxCodes)
...
app.put("/api/tax-codes/:id", async (c) => {
  ...
  const [existing] = await db.select().from(schema.taxCodes)
    .where(and(eq(schema.taxCodes.id, id), isNull(schema.taxCodes.deletedAt)));
  if (!existing) return c.json({ error: "Not found." }, 404);
  ...
  const [result] = await db.update(schema.taxCodes).set(update)
    .where(eq(schema.taxCodes.id, id)).returning();
  if (!result) return c.json({ error: "Internal error" }, 500);
  await withAudit(db, schema.auditLog, {
    actorUserId: caller.userId,
    action: "edit",
